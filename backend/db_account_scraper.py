#!/usr/bin/env python3
"""
Database-based Twitter scraper that uses accounts from MongoDB instead of SQLite
"""
...
class DatabaseTwitterScraper:
    ...
    async def setup_accounts_from_db_data(self, accounts_data: List[Dict]) -> bool:
        """Setup accounts from database data (passed from Node.js API)"""
        log.info(f"Setting up {len(accounts_data)} accounts from database...")
        
        for i, account_data in enumerate(accounts_data):
            try:
                cookies = account_data.get('cookies', [])
                account_name = account_data.get('twitterAccountName', f'DB_Account_{i+1}')
                
                if not cookies:
                    log.warning(f"No cookies found for account {account_name}")
                    continue
                
                # Create a temporary in-memory pool for this account
                pool = AccountsPool()
                api = API(pool)
                
                temp_username = f"_db_user_{uuid.uuid4().hex[:8]}"
                
                # Convert cookies to the format twscrape expects
                if isinstance(cookies, list) and len(cookies) > 0:
                    # If cookies are already in the right format
                    if isinstance(cookies[0], dict) and 'name' in cookies[0]:
                        cookies_dict = {cookie['name']: cookie['value'] for cookie in cookies}
                    else:
                        # If cookies are in a different format, try to use as-is
...
async def check_dm_availability(api: API, user_id: str) -> Optional[bool]:
    try:
        ...
        if response.status_code == 200:
            ...
            # Try the new GraphQL structure first (dm_permissions.can_dm)
...
async def scrape_with_database_accounts(username: str, limit: int, accounts_data: List[Dict]) -> List[Dict[str, Any]]:
    """Main scraping function using accounts from database"""
    scraper = DatabaseTwitterScraper()
...
async def main():
    parser = argparse.ArgumentParser(description="Database-based Twitter follower scraper")
    ...
    try:
        ...
        log.info(f"Starting database-based scraper for @{username}, target: {args.limit} followers")
        log.info(f"Using {len(accounts_data)} Twitter accounts from database")
        
        followers = await scrape_with_database_accounts(username, args.limit, accounts_data)
